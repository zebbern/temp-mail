        ...
        try:
            ...
            async with aiohttp.ClientSession(headers=self._default_headers()) as session:
                async with session.get(self.BASE_URL, params=params) as resp:
                    ...
                    # Debug the full response to understand what we're getting
                    logging.debug(f"Guerrilla Mail Response: {data}")
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(f"{self.BASE_URL}/messages/{message_id}", headers=headers) as resp:
                    ...
                    logging.debug(f"Mail.gw Response: {msg}")
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(f"{self.BASE_URL}/messages/{message_id}", headers=headers) as resp:
                    ...
                    # Debug the full response
                    logging.debug(f"Mail.tm Response: {msg}")
                ...
                for i, msg in enumerate(messages):
                    ...
                    if msg_id not in existing_ids:
                        ...
                        normalized_msg = {
                            ...
                            'mail_date': received_time,  # Use current time
